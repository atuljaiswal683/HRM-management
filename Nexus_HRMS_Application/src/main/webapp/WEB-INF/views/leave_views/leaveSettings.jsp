								...
								<div class="card shadow-sm h-100">
									<div
										...
										<h5 class="card-title mb-0"
											...
											${leave.leaveTypeName}</h5>
										<div class="custom-control custom-switch ml-3 flex-shrink-0">
											<input type="checkbox"
												class="custom-control-input status-switch"
												id="switch${leave.leaveTypeId}"
												data-leaveid="${leave.leaveTypeId}"
												${leave.status == 'ACTIVE' ? 'checked' : ''} /> <label
												class="custom-control-label"
												for="switch${leave.leaveTypeId}"></label>
										</div>
									</div>
								</div>
							</div>
...
$(document).ready(function() {
    $('.status-switch').on('change', function() {
        var leaveTypeId = $(this).data('leaveid');
        var status = $(this).prop('checked') ? 'ACTIVE' : 'INACTIVE';

        $.post('LeaveSettingsServlet', { leaveTypeId: leaveTypeId, status: status }, function() {
