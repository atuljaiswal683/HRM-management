            ...
            <table class="table table-bordered table-hover">
              ...
              <%
                try (Connection conn = util.DatabaseConnection.getConnection();
                     PreparedStatement ps = conn.prepareStatement("SELECT user_id, first_name, last_name FROM users WHERE role_id != 0");
              ...
              %>
                <tr>
                  ...
                  <td>
                    ...
                    <form method="post" action="<%= request.getContextPath() %>/Admin_AssignedTicket" class="d-flex gap-1">
                        <input type="hidden" name="ticketId" value="<%= t.getTicketId() %>">
                        <select name="staffUserId" class="form-select form-select-sm" required>
                            <option value="">--Select Employee--</option>
                            <% for (Users u : employees) { %>
                                <option value="<%= u.getUserId() %>" <%= (t.getAssignTo() == u.getUserId() ? "selected" : "") %>>
